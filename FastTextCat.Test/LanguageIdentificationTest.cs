...
namespace FastTextCat.Test
{
    ...
    {
        private string _identifierFile = Path.Combine("..", "..","..", "..", "FastTextCat", "LanguageModels", "WiLI-2018.xml");

        [Test]
        public void TestLanguageClassifierManager()
        {
            var Testory = new LanguageClassifierManager();
            var identifier = Testory.LoadLanguageModelsAndCreateClassifier(_identifierFile);

            var res = identifier.Identify("был зачитан вслух");
            Assert.AreEqual("rus", res.First().Category.Iso639_2T);

            res = identifier.Identify("What is the meaning of this");
            Assert.AreEqual("eng", res.First().Category.Iso639_2T);

            res = identifier.Identify("Главная задача сэмпла - предоставить желающим качать возможность оценить реальное качество материала без скачивания всей раздачи целиком. Поэтому вырезать сэмпл надо из середины фильма и без каких либо искажений. Достаточно фрагмента на 1-2 минуты. Заливать сэмпл следует только на файлообменники");
            Assert.AreEqual("rus", res.First().Category.Iso639_2T);
        }


        [Test]
        public void TestTrainIdentifyCycle()
        {
            TestCase[] testCases = PrepareTestCases();
            var Testory = new LanguageClassifierManager();
            var input = testCases.Select(
                t => Tuple.Create(new LanguageInfo(t.ISO639_2T, "", "", ""), 
                (TextReader)new StringReader(t.Standard)));
            var identifier = Testory.TrainLanguageModelsAndCreateClassifier(input);

            foreach (var testCase in testCases)
            {
                Assert.AreEqual(identifier.Identify(testCase.Query).First().Category.Iso639_2T, testCase.ISO639_2T);
            }
        }
        ...
        {
            public string ISO639_2T { get; init; } = "";
